...
def create_attempt(user_id, quiz_id, attempt_type, nombre=None):
    now = datetime.utcnow().isoformat()
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            """
            INSERT INTO attempts (user_id, quiz_id, attempt_type, started_at, nombre)
            VALUES (?, ?, ?, ?, ?)
            """,
...
def obtener_intento_pendiente_por_tipo(user_id, tipo_intento):
    ...
    with get_conn() as conn:
        ...
        cur.execute(
            f"""
            SELECT id, correct, wrong, attempt_type, nombre
            FROM attempts
            WHERE user_id = ?
        ...
        return {
            ...
            "correct": fila["correct"],
            "wrong": fila["wrong"],
            "attempt_type": fila["attempt_type"],
            "nombre": fila["nombre"],
        }
...
def reconstruir_quiz_desde_db(user_id, quiz_id, telegram_user_id):
    ...
    if not intento:
        return None
    questions = obtener_test_temporal(intento["id"])
    if not questions:
...
async def iniciar_quiz(
    ...
    nombre_intento = obtener_titulo_test(quiz_id) if quiz_id else None
    attempt_id = create_attempt(user_id, quiz_id, attempt_type, nombre=nombre_intento)
    guardar_test_temporal(attempt_id, questions)
...
async def iniciar_test_temporal(
    ...
    cerrar_intento_en_curso(context)
    nombre_intento = "Test de fallos" if tipo_intento == "failures" else "Test de favoritas"
    attempt_id = create_attempt(user_id, None, tipo_intento, nombre=nombre_intento)
