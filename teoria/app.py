...
# Conexión directa a Postgres para pg_dump / pg_restore. Se pasan por
# variables de entorno estándar de libpq — así pg_dump las coge sin más.
PGHOST = os.getenv("PGHOST", "db")
PGPORT = os.getenv("PGPORT", "5432")
PGUSER = os.getenv("PGUSER", "aprentix")
PGDATABASE = os.getenv("PGDATABASE", "aprentix")

# Extensión propia del backup total (7z encriptado con AES-256 + cabecera
# cifrada). Un fichero .apbak abarca dump de BBDD + ficheros de teoría +
...
# ── Backup / restauración total ────────────────────────────────────────────
#
# El admin puede descargarse un fichero .apbak con TODO el estado de la
# aplicación (BBDD + ficheros de teoría + README con las instrucciones de
# restauración) y cargarlo en otro servidor para dejarlo funcional sin
# pasos manuales. El .apbak es un 7z encriptado con AES-256 (cabecera y
# nombres de fichero incluidos) protegido por una contraseña que elige
# el admin en el momento de generarlo.

# README que se empaqueta dentro del propio .apbak. Sirve como manual
# rápido para el operador que reciba el backup: le explica qué contiene y
# cómo restaurarlo, tanto desde el propio panel de admin como a mano.
BACKUP_README = """# Backup total de Aprentix ({fecha})

Fichero: `{nombre}`
Formato: 7z (AES-256, cabecera cifrada). Extensión propia: `.apbak`.

## Contenido

```
README.md              ← este fichero
db/aprentix.dump       ← pg_dump -Fc de la BBDD `aprentix`
teoria/                ← copia íntegra de /mnt/data/ficheros
```

## Restauración rápida (recomendada)

En el servidor destino, con la plataforma ya desplegada en Dokploy y con
el usuario `admin` creado (usa la `ADMIN_PASS` del stack `core`):

1. Entra en `https://<tu-dominio>/teoria/` como `admin`.
2. Menú avatar → «Panel de administración» → «Restaurar backup».
3. Selecciona este fichero `.apbak` y escribe la contraseña con la que
   fue generado.
4. Espera al mensaje de confirmación. La aplicación se restaurará
   entera: BBDD + ficheros de teoría.

## Restauración manual

Si no puedes usar el panel (por ejemplo, la BBDD está vacía y aún no hay
usuario `admin`), restaura primero solo la BBDD desde el host del VPS:

```bash
# 1) Descomprime el .apbak (te pedirá la contraseña).
7z x aprentix-backup.apbak -oaprentix-backup

# 2) Restaura la BBDD (dentro del contenedor `db` del stack `core`).
docker compose -f deploy/core/docker-compose.yml exec -T db \\
    pg_restore --clean --if-exists -U aprentix -d aprentix \\
    < aprentix-backup/db/aprentix.dump

# 3) Restaura los ficheros de teoría en el host.
sudo rm -rf /mnt/data/ficheros/*
sudo cp -a aprentix-backup/teoria/. /mnt/data/ficheros/
sudo chown -R 1000:1000 /mnt/data/ficheros
```

## Preparación previa (VPS vacío)

Si el destino es un servidor nuevo:

1. Sigue `DESPLIEGUE.md` hasta tener los stacks `core`, `app` (y
   opcionalmente `notificador`) desplegados en Dokploy y funcionando.
2. Crea las carpetas del host:
   ```bash
   sudo mkdir -p /mnt/data/pg /mnt/data/ficheros /mnt/data/embeddings_cache
   ```
3. Asegúrate de que `JWT_SECRET` y `DB_PASS` son los mismos que en el
   servidor origen si quieres conservar todas las contraseñas y tokens
   activos; si los cambias, los usuarios tendrán que volver a loguearse
   pero el backup se restaura igual.
4. Restaura este `.apbak` desde el panel de admin (o manualmente,
   siguiendo la sección anterior).
"""


def _pgpassword_env() -> dict:
    """Devuelve el entorno para pg_dump / pg_restore.

    Se prefiere PGPASSWORD si viene en el entorno; si no, no se añade
    (útil si en algún momento se pasa a .pgpass).
    """
    env = os.environ.copy()
    # Homogeneizamos: aunque el operador solo haya definido PGPASSWORD,
    # se pasa tal cual; pg_dump respeta libpq.
    return env


def _run(cmd: list[str], *, stdout=None, stdin=None, check=True, env=None) -> int:
    """Ejecuta un subproceso y lanza HTTP 500 con la salida si falla.

    Los errores concretos (pg_dump/7z) suelen ser cruciales para
    diagnosticar problemas — se devuelven al cliente para que el admin
    los vea en pantalla en vez de tener que abrir logs.
    """
    proc = subprocess.run(
        cmd,
    ...
    if check and proc.returncode != 0:
        err = (proc.stderr or b"").decode("utf-8", "replace").strip() or f"{cmd[0]} salió con {proc.returncode}"
        raise HTTPException(status_code=500, detail=f"{cmd[0]}: {err[:1500]}")
    return proc.returncode


def _crear_apbak(destino: Path, workdir: Path, password: str) -> None:
    """Empaqueta el contenido de `workdir` en `destino` como 7z cifrado.

    Usa `7z a -p<pass> -mhe=on` para que también los nombres de fichero
    queden cifrados; solo con la contraseña correcta se puede listar el
    contenido, así el backup no filtra ni siquiera qué carpetas de
    teoría existen.
    """
    # `-p` sin espacio: 7z lee la contraseña del propio argumento.
    # `-mhe=on`: header encryption (nombres de fichero también cifrados).
    # `-mx=5`: compresión estándar (buena relación tamaño/tiempo).
    # `.` como patrón para incluir todo el contenido de la carpeta actual.
    _run(
        ["7z", "a", f"-p{password}", "-mhe=on", "-mx=5", str(destino), "."],
        stdout=subprocess.DEVNULL,
    )


def _extraer_apbak(origen: Path, destino: Path, password: str) -> None:
    """Extrae un .apbak en `destino`. HTTP 400 si la contraseña no vale."""
    proc = subprocess.run(
        ["7z", "x", f"-p{password}", f"-o{destino}", "-y", str(origen)],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.PIPE,
    )
    if proc.returncode != 0:
        err = (proc.stderr or b"").decode("utf-8", "replace").strip()
        # Contraseña incorrecta / fichero no es 7z / cabecera corrupta.
        if "Wrong password" in err or "wrong password" in err or "Data Error" in err:
            raise HTTPException(status_code=400, detail="contrasena_incorrecta_o_fichero_corrupto")
        raise HTTPException(status_code=400, detail=f"backup_invalido: {err[:400]}")


@app.post("/api/backup")
def api_backup(request: Request, password: str = Form(...)):
    """Genera un .apbak con dump de BBDD + ficheros de teoría y lo
    devuelve como descarga. La contraseña la elige el admin en el
    momento — sin ella el archivo no se puede abrir ni recuperar."""
    require_admin(request)
    if not password or len(password) < 8:
        raise HTTPException(status_code=400, detail="password_debe_tener_8_o_mas_caracteres")

    # tempdir se crea con mkdtemp para poder devolverlo al cliente en
    # streaming y limpiarlo cuando FastAPI cierre el response. Si usara
    # TemporaryDirectory como contexto, se borraría antes de que el
    # cliente pudiera descargarlo.
    tmp_root = Path(tempfile.mkdtemp(prefix="apbak_"))
    workdir = tmp_root / "contenido"
    workdir.mkdir(parents=True)
    (workdir / "db").mkdir()

    try:
        # 1) Dump de la BBDD en formato custom (comprimido, restaurable
        # selectivamente con pg_restore).
        dump_path = workdir / "db" / "aprentix.dump"
        ...
        # 2) Copia íntegra de /ficheros dentro del workdir. Se copia
        # (en vez de referenciar) para que 7z pueda comprimirlos con
        # rutas relativas dentro del backup.
        teoria_dst = workdir / "teoria"
        if BASE_DIR.exists():
            shutil.copytree(BASE_DIR, teoria_dst, symlinks=False)
        else:
            teoria_dst.mkdir()

        # 3) README con instrucciones de restauración.
        fecha = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
        nombre_out = f"aprentix-backup-{datetime.utcnow():%Y%m%d-%H%M%S}{BACKUP_EXT}"
        (workdir / "README.md").write_text(
            BACKUP_README.format(fecha=fecha, nombre=nombre_out),
            encoding="utf-8",
        )

        # 4) Comprimimos con 7z cifrado. Guardamos el .apbak fuera del
        # workdir para que no se autoincluya.
        apbak_path = tmp_root / nombre_out
        # 7z entra en `workdir` (cwd) y comprime "." → así el archivo
        # queda con `README.md`, `db/`, `teoria/` en la raíz.
        subprocess.run(
            ["7z", "a", f"-p{password}", "-mhe=on", "-mx=5", str(apbak_path), "."],
            cwd=workdir, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, check=True,
        )
        ...
        shutil.rmtree(tmp_root, ignore_errors=True)
        raise HTTPException(status_code=500, detail=f"backup_error: {e}")

    # Se libera el tmp_root en cuanto la respuesta se termina de enviar.
    def _limpiar():
        shutil.rmtree(tmp_root, ignore_errors=True)
    ...
    return FileResponse(
        ...
        filename=nombre_out,
        background=BackgroundTask(_limpiar),
        headers={"Content-Disposition": f'attachment; filename="{nombre_out}"'},
    )
...
async def api_restore(
    ...
    """Restaura un .apbak: sustituye la BBDD y los ficheros de teoría.
    La operación es DESTRUCTIVA: sobrescribe el estado actual del
    servidor con el del backup."""
    require_admin(request)
    if not password:
        raise HTTPException(status_code=400, detail="password_requerida")

    tmp_root = Path(tempfile.mkdtemp(prefix="aprestore_"))
    try:
        # 1) Guardar el fichero subido a disco (evita cargarlo entero en
        # RAM aunque sea grande — 7z lo lee del disco).
        upload_path = tmp_root / "backup.apbak"
        ...
        # 2) Descomprimir. Si la contraseña falla, HTTP 400 con detalle.
        extract_dir = tmp_root / "contenido"
        extract_dir.mkdir()
        _extraer_apbak(upload_path, extract_dir, password)

        dump_path = extract_dir / "db" / "aprentix.dump"
        teoria_src = extract_dir / "teoria"
        if not dump_path.exists():
            raise HTTPException(status_code=400, detail="backup_sin_dump_de_bbdd")
        if not teoria_src.exists() or not teoria_src.is_dir():
            raise HTTPException(status_code=400, detail="backup_sin_carpeta_teoria")

        # 3) Restaurar la BBDD. `--clean --if-exists` deja el schema como
        # si estuviera vacío antes de meter los datos, así se puede
        # cargar sobre una BBDD ya inicializada por 01_esquema.sql sin
        # colisiones. `--no-owner` evita fallar si el rol destino no es
        # exactamente `aprentix`.
        _run(
            [
                "pg_restore",
        ...
        # 4) Reemplazar el contenido de /ficheros por el del backup.
        # Se borra el contenido pero NO el propio directorio (es un
        # bind-mount desde el host; borrarlo dejaría el volumen huérfano).
        for hijo in BASE_DIR.iterdir():
            if hijo.is_dir() and not hijo.is_symlink():
                shutil.rmtree(hijo)
            else:
                hijo.unlink()
        for hijo in teoria_src.iterdir():
            destino = BASE_DIR / hijo.name
            if hijo.is_dir():
                shutil.copytree(hijo, destino, symlinks=False)
            else:
                shutil.copy2(hijo, destino)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"restore_error: {e}")
    finally:
        shutil.rmtree(tmp_root, ignore_errors=True)

    return {
        "ok": True,
        "mensaje": "Backup restaurado. BBDD y ficheros de teoría actualizados.",
    }
