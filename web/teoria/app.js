...
/* ── Backup / restauración total (solo admin) ─────────────────────────
 * Dispara la descarga de un fichero .apbak (7z cifrado con AES-256 y
 * cabecera cifrada, ver /api/backup) protegido por la contraseña que
 * elija el admin.  El backup incluye el dump de la BBDD, todos los
 * ficheros de teoría y un README con las instrucciones para restaurarlo
 * en un servidor nuevo. */
function abrirBackupDialog() {
  if (!document.body.classList.contains('es-admin')) {
    toast('Solo el admin puede generar backups');
    return;
  }
  const old = document.getElementById('modal-host');
  const host = old.cloneNode(false); host.id = 'modal-host';
  old.parentNode.replaceChild(host, old);
  const escT = (s) => String(s ?? '').replace(/[&<>"']/g, c => (
    { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]
  ));
  host.innerHTML = `
    <div class="modal">
      <div class="modal-card">
        ...
        <div class="modal-body">
          <p class="muted small">
            Se generará un fichero <code>.apbak</code> con el dump de la
            base de datos, todos los ficheros de teoría y un README con
            los pasos para restaurar. El archivo se cifra con AES-256
            (incluyendo los nombres de fichero) usando la contraseña
            que indiques aquí; guárdala junto al backup — sin ella no
            se puede recuperar.
          </p>
          <label for="backup-pass" style="display:block; margin:.75rem 0 .25rem; font-size:.9rem;">Contraseña de cifrado (mínimo 8 caracteres)</label>
          <input id="backup-pass" type="password" autocomplete="new-password" minlength="8" placeholder="Elige una contraseña fuerte" style="width:100%; box-sizing:border-box;">
          <label for="backup-pass2" style="display:block; margin:.75rem 0 .25rem; font-size:.9rem;">Repite la contraseña</label>
          <input id="backup-pass2" type="password" autocomplete="new-password" minlength="8" style="width:100%; box-sizing:border-box;">
          <p id="backup-status" class="muted small" hidden></p>
          <div class="modal-actions">
            <button class="btn btn-cancel" data-a="cancel">Cancelar</button>
            <button class="btn btn-pri" data-a="ok" id="backup-ok">Generar y descargar</button>
          </div>
        </div>
      </div>
    </div>`;
  const pass  = host.querySelector('#backup-pass');
  const pass2 = host.querySelector('#backup-pass2');
  const status = host.querySelector('#backup-status');
  const ok = host.querySelector('#backup-ok');
  pass.focus();
  const cerrar = () => { host.innerHTML = ''; };
  host.addEventListener('click', async (e) => {
    if (e.target.classList.contains('modal') || e.target.closest('[data-a="cancel"]')) { cerrar(); return; }
    if (!e.target.closest('[data-a="ok"]')) return;
    const p1 = pass.value, p2 = pass2.value;
    if (!p1 || p1.length < 8) { status.hidden = false; status.textContent = 'La contraseña debe tener al menos 8 caracteres.'; return; }
    if (p1 !== p2) { status.hidden = false; status.textContent = 'Las contraseñas no coinciden.'; return; }
    ok.disabled = true;
    status.hidden = false;
    status.textContent = 'Generando backup… esto puede tardar según el tamaño de la teoría.';
    try {
      const fd = new FormData();
      fd.append('password', p1);
      const r = await fetch('/teoria/api/backup', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${TOKEN}` },
        body: fd,
      });
      ...
      // Extrae el nombre del Content-Disposition (o inventa uno).
      const cd = r.headers.get('Content-Disposition') || '';
      let filename = 'aprentix-backup.apbak';
      ...
      status.textContent = `Backup listo: ${filename} (${fmtSize(blob.size)}). Guarda la contraseña.`;
...
function abrirRestoreDialog() {
  if (!document.body.classList.contains('es-admin')) {
    toast('Solo el admin puede restaurar backups');
    return;
  }
  const old = document.getElementById('modal-host');
  const host = old.cloneNode(false); host.id = 'modal-host';
  ...
  host.innerHTML = `
    <div class="modal">
      <div class="modal-card">
        ...
        <div class="modal-body">
          <p class="muted small" style="color: var(--color-danger, #b23);">
            ⚠️ Esta operación es <strong>destructiva</strong>. Reemplaza
            la base de datos y los ficheros de teoría por los contenidos
            del backup. Todo lo que haya ahora se perderá.
          </p>
          <label for="restore-file" style="display:block; margin:.75rem 0 .25rem; font-size:.9rem;">Fichero <code>.apbak</code></label>
          <input id="restore-file" type="file" accept=".apbak,application/octet-stream" style="width:100%; box-sizing:border-box;">
          <label for="restore-pass" style="display:block; margin:.75rem 0 .25rem; font-size:.9rem;">Contraseña del backup</label>
          <input id="restore-pass" type="password" autocomplete="current-password" style="width:100%; box-sizing:border-box;">
          <p id="restore-status" class="muted small" hidden></p>
          <div class="modal-actions">
            <button class="btn btn-cancel" data-a="cancel">Cancelar</button>
            <button class="btn btn-danger" data-a="ok" id="restore-ok">Restaurar</button>
          </div>
        </div>
      </div>
    </div>`;
  const file = host.querySelector('#restore-file');
  const pass = host.querySelector('#restore-pass');
  const status = host.querySelector('#restore-status');
  const ok = host.querySelector('#restore-ok');
  const cerrar = () => { host.innerHTML = ''; };
  host.addEventListener('click', async (e) => {
    if (e.target.classList.contains('modal') || e.target.closest('[data-a="cancel"]')) { cerrar(); return; }
    if (!e.target.closest('[data-a="ok"]')) return;
    ...
    const p = pass.value;
    if (!f) { status.hidden = false; status.textContent = 'Selecciona un fichero .apbak.'; return; }
    if (!p) { status.hidden = false; status.textContent = 'Escribe la contraseña con la que se generó.'; return; }
    ok.disabled = true;
    status.hidden = false;
    status.textContent = 'Restaurando… no cierres esta ventana ni recargues la página.';
    try {
      const fd = new FormData();
      fd.append('password', p);
      fd.append('fichero', f);
      const r = await fetch('/teoria/api/restore', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${TOKEN}` },
        body: fd,
      });
      const data = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(data.error || `HTTP ${r.status}`);
      status.textContent = `✅ ${data.mensaje || 'Backup restaurado.'} Recargando…`;
...
/* Atajo por URL: si se llega a /teoria/?atajo=backup o ?atajo=restaurar
 * (por ejemplo desde el panel de admin común de la app de tests), se
 * abre directamente el modal correspondiente en cuanto haya sesión. */
(function _handleAtajo() {
  try {
    const a = new URLSearchParams(location.search).get('atajo');
    if (a !== 'backup' && a !== 'restaurar') return;
    // Espera a que se aplique la sesión y el body tenga es-admin.
    const abrir = () => {
      if (!document.body.classList.contains('es-admin')) return false;
      if (a === 'backup') abrirBackupDialog(); else abrirRestoreDialog();
      // Limpia el atajo del URL para que un reload no vuelva a abrirlo.
      const u = new URL(location.href);
      u.searchParams.delete('atajo');
      history.replaceState(null, '', u.toString());
      return true;
    };
    if (abrir()) return;
    const mo = new MutationObserver(() => { if (abrir()) mo.disconnect(); });
    mo.observe(document.body, { attributes: true, attributeFilter: ['class'] });
    // Salvavidas: se autolimpia a los 15 s aunque no llegue es-admin.
