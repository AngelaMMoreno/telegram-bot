...
| 2026-07-26  | `2026-07-26_login_web.sql`                | Añade columna `username` (citext UNIQUE) a `usuarios` con backfill desde la parte local del email. Crea las RPCs que la SPA ya llamaba y no existían: `iniciar_sesion`, `login_web`, `registrar_web`, `mi_sesion`, `puede_ver_teoria`, `oposiciones_publicas`, `elegir_mis_oposiciones`. El login es por **email + contraseña**; el registro pide **email + usuario + contraseña**. Desbloquea el error "Could not find the function public.login_web in the schema cache" en la pantalla de inicio de sesión. |
