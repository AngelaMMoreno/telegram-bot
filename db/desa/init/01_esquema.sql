-- =============================================================================
-- 01_esquema.sql  (Aprentix DESA — rediseño orientado a oposiciones)
--
-- Este es el esquema autoritativo para el entorno paralelo `desa`
-- (base de datos `aprentix_desa`).  Todo el modelo gira en torno a la
-- oposición y sus temas — la separación teoría/tests desaparece.
--
--   oposiciones  1─N  oposicion_temas  N─1  temas  1─N  unidades  1─N  preguntas
--
-- Temas son reutilizables entre oposiciones (una misma "Constitución
-- Española" puede aparecer en dos oposiciones distintas con el mismo
-- material).  Las unidades son la pieza atómica que combina teoría
-- (markdown) y tests (colección de preguntas).
--
-- Se ejecuta sobre una BBDD vacía (docker-entrypoint-initdb.d).  No hay
-- migraciones parciales; para evolucionar el esquema se edita este
-- fichero y se aplica manualmente contra el entorno vivo.
-- =============================================================================
...
CREATE TABLE usuarios (
    ...
    -- El email es el login; el trigger `usuarios_email_normalize` lo pone
    -- siempre en minúsculas antes de insertar, así el UNIQUE por email es
    -- case-insensitive de facto sin depender de la extensión citext.
    email                  text NOT NULL,
    nombre                 text NOT NULL,             -- nombre visible en la UI
    password_hash          text,                      -- bcrypt (pgcrypto)
    email_verificado       boolean NOT NULL DEFAULT false,
    activo                 boolean NOT NULL DEFAULT true,
    avatar                 text,                       -- URL o inicial calculada en la SPA
    creado_en              timestamptz NOT NULL DEFAULT now(),
    UNIQUE (email)
);

-- El nombre visible por defecto y el "handle" para menciones no son
-- login: el login es el email.  Mantener email en minúsculas.
...
CREATE TABLE permisos (
    id          text PRIMARY KEY,            -- 'oposicion.crear', 'test.responder'…
    descripcion text
);
...
-- Tokens para acciones asíncronas por email (confirmar cuenta, reset password).
-- Un solo token vivo por (usuario, tipo).  El worker de mailer no toca esta
-- tabla: los tokens los emite/consume la propia BBDD.
...
CREATE INDEX email_tokens_usuario_idx ON email_tokens (usuario_id, tipo)
...
--                              COLA DE EMAILS
-- =============================================================================
-- La BBDD sólo encola.  El servicio `mailer` (worker Python) lee filas
-- pendientes y las envía por SMTP.  Guardamos también `error` para poder
-- reintentar / auditar desde pgAdmin sin salir de la BBDD.
...
CREATE TABLE oposiciones (
    id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    slug         text UNIQUE NOT NULL,      -- 'auxiliar-administrativo'
    nombre       text NOT NULL,
    descripcion  text,
    organismo    text,                       -- convocante (AGE, JCyL, ...)
    activa       boolean NOT NULL DEFAULT true,
    creada_en    timestamptz NOT NULL DEFAULT now(),
    autor_id     uuid REFERENCES usuarios(id) ON DELETE SET NULL
);

-- Un TEMA es reutilizable: la "Constitución Española" puede formar parte
-- de varias oposiciones.  Por eso vive en su propia tabla y se enlaza a
-- las oposiciones vía la tabla puente `oposicion_temas`.
CREATE TABLE temas (
    id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    slug         text UNIQUE NOT NULL,       -- 'constitucion-espanola'
    nombre       text NOT NULL,
    descripcion  text,
    icono        text NOT NULL DEFAULT '📘', -- se muestra en las tarjetas
    creado_en    timestamptz NOT NULL DEFAULT now(),
    autor_id     uuid REFERENCES usuarios(id) ON DELETE SET NULL
);

-- Puente N:M.  `orden` fija la posición del tema dentro de esa oposición
-- (así "Constitución" puede ser el tema 1 en una oposición y el tema 3
-- en otra sin duplicar el material).
...
-- puede tener preguntas asociadas para el test de esa unidad.  Se
-- comparten entre oposiciones porque cuelgan del tema, no de la
-- oposición.
CREATE TABLE unidades (
    id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tema_id       uuid NOT NULL REFERENCES temas(id) ON DELETE CASCADE,
    slug          text NOT NULL,             -- único dentro del tema
    nombre        text NOT NULL,
    orden         int  NOT NULL,
    teoria_md     text NOT NULL DEFAULT '',  -- markdown; la SPA lo renderiza
    resumen       text,                       -- resumen corto para tarjetas
...
-- preguntas se reutilizan también (no hay que reimportarlas).
CREATE TABLE preguntas (
    id             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    unidad_id      uuid NOT NULL REFERENCES unidades(id) ON DELETE CASCADE,
    enunciado      text NOT NULL,
    -- Formato canónico: [{"texto": "…", "correcta": true|false}, …]
...
CREATE TABLE usuario_oposiciones (
    ...
    principal     boolean NOT NULL DEFAULT false,   -- la oposición "activa" en la UI
    PRIMARY KEY (usuario_id, oposicion_id)
);
-- Solo puede haber una principal por usuario a la vez.
CREATE UNIQUE INDEX uo_principal_unica_idx ON usuario_oposiciones (usuario_id)
    WHERE principal;
...
-- Un INTENTO = una sesión de resolución de test dentro de una unidad.
CREATE TABLE intentos (
    id             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    usuario_id     uuid NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
    unidad_id      uuid NOT NULL REFERENCES unidades(id) ON DELETE CASCADE,
    -- Orden congelado para poder reanudar sin depender de cambios
    -- posteriores en las preguntas de la unidad.
...
CREATE TABLE respuestas (
    ...
    opcion_elegida text NOT NULL,                    -- texto de la opción
    correcta       boolean NOT NULL,
    respondida_en  timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX respuestas_intento_idx ON respuestas (intento_id);

-- Progreso agregado por (usuario, unidad).  Se actualiza al finalizar
-- un intento y cuando el usuario marca la teoría como leída.
...
CREATE INDEX progreso_unidad_uid_idx ON progreso_unidad (usuario_id);

-- Marcadores unificados (fallos y favoritas).
...
--                          PLAN DE ESTUDIO
-- =============================================================================
-- La generación del plan se hará más adelante — ahora la BBDD ya guarda
-- las respuestas del wizard (disponibilidad, fecha del examen, ritmo…)
-- y las sesiones diarias que el motor produzca.
...
CREATE TABLE plan_estudio (
    ...
    -- Array de 7 booleans (lun..dom) con qué días del semana estudia.
...
-- Sesiones concretas que el motor genera para cada día.  De momento la
-- SPA las muestra "vacías" — el motor de planificación real se
-- implementará en un iteración posterior.
...
-- =============================================================================
-- Retos con periodo (diario/semanal/mensual): el motor NO necesita reset
-- porque la clave incluye el `periodo_inicio` (fecha del lunes / día /
-- primer día de mes).  Cambiar de periodo crea una fila nueva.
...
CREATE TABLE push_envios (
    usuario_id  uuid NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
    tipo        text NOT NULL CHECK (tipo IN ('recordatorio','reto','inactividad','prueba')),
...
-- =============================================================================
-- Igual que en el esquema principal: un único rol de conexión
-- `autenticador` para PostgREST; identidad y roles funcionales vía JWT.
...
-- Lectura del catálogo básico incluso sin sesión (para poder registrar).
GRANT SELECT ON roles, permisos, rol_permisos TO web_anon, web_user;

-- Contenido: lectura para autenticados; escritura vía RLS + permisos.
...
-- =============================================================================
-- Firma JWT HS256 en SQL puro sobre pgcrypto (misma técnica que el
-- esquema principal — evita depender de pgjwt).
...
-- Usuarios: cada uno se ve a sí mismo; admin todos.
...
-- Oposiciones: lectura para autenticados; escritura para 'oposicion.gestionar' o admin.
...
-- Temas: lectura para autenticados; escritura para 'oposicion.gestionar'.
...
-- Unidades: lectura autenticados; escritura por permiso.
...
-- Preguntas: lectura autenticados; escritura por permiso.
...
-- Matrícula del usuario en oposiciones (solo suyas; admin ve todas).
...
-- Actividad: cada usuario ve/edita sólo lo suyo.
...
-- Plan de estudio (usuario o admin).
...
-- Catálogos gamificación: lectura autenticados; escritura admin.
...
-- Push: el usuario gestiona los suyos.
...
-- =============================================================================
-- Los INSERT desde la SPA no necesitan enviar usuario_id: lo aporta el JWT.
...
--                                 AUTENTICACIÓN
-- =============================================================================
-- El login es el email.  El registro sólo crea la cuenta y encola el
-- correo de verificación: hasta confirmarlo, el usuario no puede
-- iniciar sesión (`iniciar_sesion` lo comprueba).

-- URL base pública de la app: se lee de config('app_url') para poder
-- construir el enlace de verificación tanto en dev como en desa/prod
-- sin hard-codear el dominio en el SQL.
CREATE OR REPLACE FUNCTION app_url() RETURNS text
LANGUAGE sql STABLE AS $$
    SELECT COALESCE(
        (SELECT valor #>> '{}' FROM config WHERE clave = 'app_url'),
        'https://desa.aprentix.es'
    );
$$;
...
BEGIN
    ...
    -- Cada nuevo usuario es 'alumno' por defecto.  El rol 'admin' se
    -- promociona a mano desde la BBDD.
...
BEGIN
    ...
    -- Silenciosamente OK aunque el email no exista, para no filtrar cuentas.
...
--     "nombre": "Auxiliar Administrativo (AGE)",
--     "descripcion": "...",
--     "organismo": "AGE",
--     "temas": [
--       {
--         "slug": "constitucion-espanola",           -- si ya existe, se reutiliza
--         "nombre": "Constitución Española",
--         "icono": "⚖️",
--         "orden": 1,
--         "unidades": [                                -- solo se crean si el tema es nuevo
--           {
--             "slug": "titulo-preliminar",
--             "nombre": "Título preliminar",
--             "orden": 1,
--             "teoria_md": "# Título preliminar\n\n...",
--             "minutos_est": 25,
--             "preguntas": [
--               {
--                 "enunciado": "¿Qué establece el artículo 1?",
--                 "opciones": [
--                   {"texto": "España es un Estado social...", "correcta": true},
--                   {"texto": "...", "correcta": false}
--                 ],
--                 "explicacion": "Artículo 1.1 CE."
--               }
--             ]
--           }
--         ]
...
--   }
--
-- Si un tema ya existe (mismo slug) NO se sobreescriben sus unidades ni
-- preguntas — sólo se vincula a la oposición en el orden indicado.  Así
-- una oposición nueva puede reutilizar temas ya cargados sin duplicar
-- material.
...
BEGIN
    ...
    -- Upsert de la oposición por slug.
    ...
    -- Los temas anteriores de esta oposición se quitan del puente (no de
    -- la tabla temas — otras oposiciones pueden usarlos).  Luego se
    -- vuelven a enganchar en el orden que trae el payload.
    ...
    LOOP
        v_orden := COALESCE((v_tema->>'orden')::int,
                            (SELECT COALESCE(max(orden), 0) + 1 FROM oposicion_temas WHERE oposicion_id = v_opos_id));
        ...
        -- Solo se importan unidades si el tema es NUEVO — así no
        -- pisamos el material existente al reutilizar un tema.
        IF v_tema_new THEN
            ...
            LOOP
                ...
                VALUES (
                    ...
                    COALESCE((v_unidad->>'orden')::int,
                             (SELECT COALESCE(max(orden), 0) + 1 FROM unidades WHERE tema_id = v_tema_id)),
...
-- =============================================================================
--              CATÁLOGO DE OPOSICIONES, TEMAS, UNIDADES (LECTURA)
-- =============================================================================
    ...
    RETURN jsonb_build_object(
        ...
        'preguntas',  (
            SELECT jsonb_agg(jsonb_build_object(
                ...
                -- Se quitan las flags 'correcta' para que la UI no las lea.
...
DECLARE
    ...
    v_correcta boolean;
BEGIN
    -- El propietario del intento se comprueba por RLS.
...
BEGIN
    ...
    -- Gamificación: sumar 10 XP por acierto y actualizar racha.
    UPDATE usuario_gamificacion
       SET xp_total = xp_total + (v_correctas * 10),
           racha_actual = CASE
               WHEN ultimo_dia_activo = current_date THEN racha_actual
               WHEN ultimo_dia_activo = current_date - 1 THEN racha_actual + 1
               ELSE 1
           END,
           racha_maxima = GREATEST(racha_maxima,
               CASE WHEN ultimo_dia_activo = current_date THEN racha_actual
                    WHEN ultimo_dia_activo = current_date - 1 THEN racha_actual + 1
                    ELSE 1 END),
...
LANGUAGE sql AS $$
    ...
    VALUES (jwt_usuario_id(), p_unidad_id, p_completada, CASE WHEN p_completada THEN now() ELSE NULL END, now())
    ON CONFLICT (usuario_id, unidad_id) DO UPDATE
...
-- Datos para la pantalla "Inicio" (mockup 1).
...
LANGUAGE sql STABLE AS $$
    ...
    gamif AS (
        SELECT COALESCE(g.xp_total, 0) AS xp,
               ...
               1 + COALESCE(g.xp_total, 0) / 500 AS nivel   -- 500 XP por nivel
...
-- Datos para la pantalla "Estadísticas" (mockup 3).
...
-- Datos para la pantalla "Perfil" (mockup 4).
...
-- Config inicial.  El dominio se puede sobreescribir tras el arranque.
INSERT INTO config(clave, valor) VALUES
    ('app_url',       '"https://desa.aprentix.es"'::jsonb),
    ('app_nombre',    '"Aprentix DESA"'::jsonb),
    ('exam_deadline', 'null'::jsonb);

-- Retos y logros de arranque.
...
-- Usuario admin inicial.  El correo también se marca ya como verificado
-- para poder entrar sin pasar por SMTP durante el bootstrap.
DO $$
...
-- Contraseña del rol de conexión de PostgREST.
DO $$ BEGIN
